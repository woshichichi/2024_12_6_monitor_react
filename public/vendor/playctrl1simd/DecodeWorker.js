...
(function (event) {
  ...
  self.JSPlayM4_RunTimeInfoCallBack = function (nPort, pstRunTimeInfo, pUser) {
    ...
    postMessage({
      function: 'RunTimeInfoCallBack',
      nRunTimeModule: nRunTimeModule,
      nStrVersion: nStrVersion,
      nFrameTimeStamp: nFrameTimeStamp,
      nFrameNum: nFrameNum,
    ...
    switch (eventData.command) {
      ...
      case 'InputData':
        ...
        while (bOpenMode && bOpenStream) {
          ...
          // 直到获取视频帧或数据不足为止
          if (
            PLAYM4_VIDEO_FRAME === ret ||
            PLAYM4_NEED_MORE_DATA === ret ||
            PLAYM4_ORDER_ERROR === ret
          ) {
            //PLAYM4_VIDEO_FRAME === ret ||  || PLAYM4_NEED_NEET_LOOP === ret
            break;
          }
        }
        break;
    ...
    if (res === PLAYM4_OK) {
      ...
      switch (oFrameInfo.frameType) {
        case AUDIO_TYPE:
          ...
          if (bWorkerPrintLog) {
            console.log(
              '<<<Worker: audio media Info: nSise:' +
                oFrameInfo.frameSize +
                ',nSampleRate:' +
                oFrameInfo.samplesPerSec +
                ',channel:' +
                oFrameInfo.channels +
                ',bitsPerSample:' +
                oFrameInfo.bitsPerSample,
            );
          }
          postMessage(
            {
        ...
        case VIDEO_TYPE:
          ...
          if (bWorkerPrintLog) {
            console.log(
              '<<<Worker: video media Info: Width:' +
                oFrameInfo.width +
                ',Height:' +
                oFrameInfo.height +
                ',timeStamp:' +
                oFrameInfo.timeStamp,
            );
          }
      ...
      if (PLAYM4_DECODE_ERROR === errorCode) {
        ...
        postMessage({
          function: 'GetRawData',
          type: '',
          data: aRawData.buffer,
          rawDataLen: rawInfo.isize,
          osd: 0,
          frameInfo: null,
          errorCode: errorCode,
        });
      ...
      //需要更多数据
      if (
        PLAYM4_NEED_MORE_DATA === errorCode ||
        PLAYM4_SYS_NOT_SUPPORT === errorCode ||
        PLAYM4_NEED_NEET_LOOP === errorCode
      ) {
        postMessage({
          function: 'GetFrameData',
          type: '',
